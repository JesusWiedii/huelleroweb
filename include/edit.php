...
if (isset($_POST['update'])) {
  ...
  if (!empty($nombre) && !empty($correo) && !empty($huella) && !empty($user)) {
    ...
    if (mysqli_num_rows($resultado) == 0) {
      $query_time="UPDATE `alert` SET `h_entry`='$h_entry',`h_departure`='$h_departure',`h_d_lunch`='$h_d_lunch',`h_e_lunch`='$h_e_lunch' WHERE id=$id";
      $result_time=mysqli_query($conn, $query_time);
      if (!empty($contrasena)) {
        $query = "UPDATE usuarios set nombre_usu = '$nombre', 
        huella='$huella', correo='$correo', estado='$deshab', 
        contrasena='$contrasena', admini='$admini', username='$user' WHERE id=$id";
        $result = mysqli_query($conn, $query);
      } else {
        $query = "UPDATE usuarios set nombre_usu = '$nombre', 
        huella='$huella', correo='$correo', estado='$deshab', 
        ...
        <form action="edit.php?id=<?php echo $_GET['id']; ?>" method="POST">
          <div class="form-group" style="margin-top: 20rem;font-size: 20px;">
            ...
            <div style="text-align: center;">
              <button style="color: #27ae60; background-color: white; width:20%;" class="bguarda btn btn-succes btn-block" name="update">Actualizar</button></div>
